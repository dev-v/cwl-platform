package com.cwl.platform.mybatis.plugin.pagination;

import com.cwl.platform.entity.Page;
import com.cwl.platform.mybatis.plugin.pagination.dialect.AbstractDialect;
import com.cwl.platform.mybatis.plugin.pagination.dialect.MysqlDialect;
import com.cwl.platform.mybatis.plugin.pagination.ext.CacheCount;
import com.cwl.platform.mybatis.plugin.pagination.ext.PageConditionParseSqlSource;
import com.cwl.platform.mybatis.plugin.pagination.ext.PageParameterHandler;
...
public class PaginationInterceptor implements Interceptor {
  ...
  private static final Map<String, MappedStatement> pageMappedStatement = new HashMap<>();
  ...
  public Object intercept(final Invocation invocation) throws Throwable {
    ...
    if (prameterObj instanceof Page<?>) {
      ...
      int count = CacheCount.getCacheCount(key);
  ...
  private final void setDatas(Object[] queryArgs, Page<?> page, MappedStatement ms, BoundSql boundSql,
    ...
    MappedStatement pms = pageMappedStatement.get(id);
    if (pms == null) {
      Builder builder = getBuilder(ms, sqlSource);
      pms = builder.build();
      pageMappedStatement.put(ms.getId(), pms);
    }
    ...
    MetaObject.forObject(pms, DEFAULT_OBJECT_FACTORY, DEFAULT_OBJECT_WRAPPER_FACTORY, DEFAULT_REFLECTOR_FACTORY)
            .setValue(SQLSOURCE_STRING, sqlSource);

    queryArgs[MAPPED_STATEMENT_INDEX] = pms;
  ...
  private final int getDataBaseCount(Connection connection, BoundSql boundSql, Page<?> page, MappedStatement ms)
          throws SQLException {
    int count = 0;
